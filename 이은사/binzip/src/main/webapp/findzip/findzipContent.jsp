...
<section>
	...
	%>		<form name="reserve" action="reserve.jsp">		
			...
			<input type="hidden" name="idx" value="<%= idx_s %>"/>
				...
				<select id="reDate" name="reDate" size="10" style="width:300px;" multiple>
					<option value="info" id="choiceDate">예약가능일 / Ctrl버튼을 눌러 선택해주세요.</option>
					<%
					String ldate = df.format(c1.getTime());
					while(c1.compareTo(c2) != 1){
						
						%>
						<option value="<%=df.format(c1.getTime())%>"><%=df.format(c1.getTime())%></option> <!-- TO-DO selected ㅊ러ㅣ 추가해야함 -->
						<%
