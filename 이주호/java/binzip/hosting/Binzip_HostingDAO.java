...
public class Binzip_HostingDAO {
	...
	//호스팅 정보입력 관련 메서드 
	   public int hostingWrite(Binzip_HostingDTO dto) {
	      try {
	         conn = binzip.db.BinzipDB.getConn();
	         String sql = "insert into binzip_host_bbs values(binzip_host_bbs_idx.nextval, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, sysdate, ?, to_date(?, 'yyyy-mm-dd'), to_date(?, 'yyyy-mm-dd'))";
	         ps = conn.prepareStatement(sql);
	         ...
	         ps.setString(12, dto.getContents());
	         ps.setString(13, dto.getHost_bbs_startdate());
	         ps.setString(14, dto.getHost_bbs_enddate());
